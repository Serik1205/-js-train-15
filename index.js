...
function roundObjectValues(obj) {
	...
	const raundeEntries = entries.map(([key, value]) => {
	...
	const raundedObj = Object.fromEntries(raundeEntries);
	return raundedObj;
...
function getFractionalPart(num) {
	...
	let getFract = Math.trunc(num);
	let getPart = num - getFract;
	getPart = Math.fround(getPart);
	return getPart;
}
...
function compareAndRound(num1, num2) {
	...
	let compareMax = Math.max(num1, num2)
	compareMax = Math.round(compareMax);
	return compareMax;
}
...
function estimateInvestment(principal, interestRate, years) {
	...
	const sumInvest = Math.round(principal * Math.pow((1 + interestRate), years));
	return sumInvest;
}
